...
//             (see scripts/gen_ntt_kat.py, which also generates
//             rtl/ntt_zetas.vh).
...
static const unsigned keccak_piln[24] = {10, 7,  11, 17, 18, 3,  5,  16,
...
static size_t s_rate(void) {
  return (sha3.mode == SHA3_512_MODE)   ? 72
         : (sha3.mode == SHAKE128_MODE) ? 168
                                        : 136;
}

static u8 s_pad(void) {
  return (sha3.mode == SHAKE128_MODE || sha3.mode == SHAKE256_MODE) ? 0x1F
                                                                    : 0x06;
}
...
u32 host_get32(u32 addr) {
  if (addr >= SHA3_BASE && addr < NTT_BASE) {
    switch (addr & 0xFF) {
    case 0x04: // STATUS
      return 1u | (sha3.absorb_done ? 0x02u : 0u) |
