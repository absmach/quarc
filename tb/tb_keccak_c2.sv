...
module tb_keccak_c2;
    ...
    reg        rst_n;
    reg        c0_req, c1_req, c2_req;
    reg [1599:0] c0_in, c1_in, c2_in;
    wire       c0_done, c1_done, c2_done;
    wire [1599:0] c0_out, c1_out, c2_out;
    ...
    initial begin
        ...
        c0_req = 0; c1_req = 0; c2_req = 0;
        c0_in = 1600'b0; c1_in = 1600'b0; c2_in = 1600'b0;
        rst_n = 0;
        #20; rst_n = 1;
        #20;
        ...
        // pulse all three requests across a posedge so the engine latches them
        c0_req = 1; c1_req = 1; c2_req = 1;
        c0_in = state; c1_in = state; c2_in = state;
        @(posedge clk);
        @(negedge clk);
        c0_req = 0; c1_req = 0; c2_req = 0;

        // wait for the last client (c2) to finish
        i = 0;
        while (!c2_done && i < 1000) begin
            @(posedge clk);
            i = i + 1;
        end
        @(posedge clk);

        $display("c0[0]=%032h c1[0]=%032h c2[0]=%032h", c0_out[31:0], c1_out[31:0], c2_out[31:0]);
        ...
        if (failures == 0) $display("PASS: all 3 clients permuted correctly");
